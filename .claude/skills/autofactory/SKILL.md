...
**This install also feeds the live pipeline UI.** `--events` writes an NDJSON
feed that `factory-ui/server.mjs` tails, so a run shows up step by step at
http://localhost:3030 under **Live run**. Override the path with
`AUTOFACTORY_EVENTS`. The flag is additive: if nothing is watching the file,
nothing changes about the run.

Only one run at a time should write a given feed file. A shrinking file is read
as a new round and resets the UI, so two concurrent runs must use different
paths.
...
```bash
cd "$AUTOFACTORY_HOME" && AUTOFACTORY_SURFACE=claude-code node packages/phase1-cli/dist/cli.js run \
  --root "<absolute path to the user's repo>" \
  --events "${AUTOFACTORY_EVENTS:-$AUTOFACTORY_HOME/../factory-ui/live.ndjson}"
```
